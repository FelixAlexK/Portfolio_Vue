...
<template>
    <section class="flex justify-center w-full min-h-full">
        <article class="flex flex-col items-start py-8 w-11/12">
            <span
                class="flex dark:bg-primary-button bg-primary-button-light shadow-2xl dark:hover:shadow-primary-button hover:shadow-primary-button-light dark:text-text text-text-light rounded-full py-1 px-4 tracking-widest uppercase align-middle whitespace-nowrap text-xl font-medium">Projekte
            </span>
            <div class="grid grid-cols-1 gap-8 py-6">
                <a class="  dark:bg-card bg-card-light shadow-xl rounded-lg transition-all ease-in-out hover:scale-95 focus:scale-95 focus-visible:outline-1 dark:outline-accent outline-accent-light "
                    ...
                    <div class="row-auto col-auto p-4">
                        ...
                        <h3 class=" font-crimson transition-all ease-in-out text-base dark:text-text text-text-light ">{{
                            project.full_name }}
                        </h3>
                        <p class="transition-all ease-in-out font-inter dark:text-text text-text-light text-sm opacity-30">
                            {{
                                project.description }}</p>


                        <div
                            class="flex flex-row w-full border-t border-opacity-10 dark:border-text border-text-light gap-8 mt-4 py-4">
                            <div
                                class="flex flex-row justify-start items-center gap-2 text-base dark:text-text text-text-light">
